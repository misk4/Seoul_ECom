...
import seoul.emergency.bbibbo.aed.AedFragment;
import seoul.emergency.bbibbo.aed.AedInfoFragment;
...
public class AedMapActivity extends AppCompatActivity
    ...
    float mylong;
    // -동주-
    ArrayList<Aed> AED_ArrayList;
    NetworkService networkService;

    MarkerOptions options;
    Marker prevMarker;
    AedFragment aedFragment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView appbar = (TextView)findViewById(R.id.appbar_aed_text);
        ...
        mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.aed_map);
        mapFragment.getMapAsync(AedMapActivity.this);

        mLocMgr = (LocationManager) getSystemService(Context.LOCATION_SERVICE);
        newPosition = (ImageButton)findViewById(R.id.new_position_aed);
        newPosition.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(AedMapActivity.this, AedMapActivity.class);
        ...
        try {
            MyLocation.LocationResult locationResult = new MyLocation.LocationResult() {
                ...
                public void gotLocation(Location location) {
                    ...
                    if (initMap(location)) {
                        ...
                        Log.d("@@", "" + mylat);
                        //AedFragment로 위도 경도 값 보내기
                        bundle.putFloat("mylatitude_aed", mylat);
                        bundle.putFloat("mylongitude_aed", mylong);
                        //리스트뷰 프래그먼트
                        aedFragment = new AedFragment();
                        aedFragment.setArguments(bundle);
                        FragmentManager fragmentManager = getFragmentManager();
                        if(!isFinishing()) {
                            FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                            fragmentTransaction.add(R.id.aedFragment_place, aedFragment);
            ...
            new AlertDialog.Builder(AedMapActivity.this)
...
//                    Toast.makeText(AedMapActivity.this,"start is null",Toast.LENGTH_SHORT).show();
//                else
...
//            }
            ...
            if(mMarkerMan==null)
                Toast.makeText(AedMapActivity.this,"Press the new position button",Toast.LENGTH_SHORT).show();
            else {
    ...
    public void onResume() {
        ...
        try {
            ...
            Toast.makeText(AedMapActivity.this,"Please connect to a network",Toast.LENGTH_SHORT).show();
        }
        registerReceiver(
    ...
    public void onPause() {
        ...
        super.onPause();
        try{unregisterReceiver(receiver3);
        ...
        mLocMgr.removeUpdates(mLocListener);}catch(IllegalArgumentException e){
    ...
    public boolean initMap(Location location) {
       ...
       /* GooglePlayServicesUtil.isGooglePlayServicesAvailable(this);
        ...
        mGoogleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(pos, 16));
        ...
        mGoogleMap.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener() {
            ...
            public void onInfoWindowClick(Marker marker) {
                ...
                aedFragment.setListView(marker.getTitle());
        ...
        mGoogleMap.setOnMarkerClickListener(new GoogleMap.OnMarkerClickListener() {
            ...
            public boolean onMarkerClick(Marker marker) {
                ...
                //aedFragment.setListView(marker.getSnippet());
                return false;
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mGoogleMap.addMarker(new MarkerOptions().position(SEOUL).title("Marker in Seoul").icon(BitmapDescriptorFactory.fromResource(R.drawable.ml)));
    ...
    public void changeFragment(Aed aed) {
        ...
        arguments.putSerializable("aed", aed);


        fragment2 = new AedInfoFragment();
        fragment2.setArguments(arguments);
        FragmentManager fragmentManager = getFragmentManager();
        if(!isFinishing()) {
            FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
            //fragmentTransaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE);
            fragmentTransaction.addToBackStack("aedList");
            fragmentTransaction.replace(R.id.aedFragment_place, fragment2);

            fragmentTransaction.commitAllowingStateLoss();
        }

    }
    private void setAEDMarkerOnMap(String lat, String lon) {
        //네트워크 설정
        networkService = ApplicationController.getInstance().getNetworkService();

        //ArrayList 초기화
        AED_ArrayList = new ArrayList<Aed>();




        //모든 AED 데이터 함수 호출
        //Call<List<Aed>> call_AED_List = networkService.getAllAed();

        try {
            //현재위치 기준 AED 데이터 함수 호출
            Call<List<Aed>> call_AED_List = networkService.getAed(lat, lon);
            Log.e("SOPT", lat + "  " + lon);
            call_AED_List.enqueue(new Callback<List<Aed>>() {
                @Override
                public void onResponse(Call<List<Aed>> call, Response<List<Aed>> response) {
                    if (response.code() == 201) {
                        //AED json 파일 불러오기
                        List<Aed> AED_List = response.body();

                        for (int i = 0; i < AED_List.size(); i++) {
                            AED_ArrayList.add(
                                    new Aed(
                                            AED_List.get(i).getId(),
                                            AED_List.get(i).getBuildAddress(),
                                            AED_List.get(i).getBuildPlace(),
                                            AED_List.get(i).getClerkTel(),
                                            AED_List.get(i).getGugun(),
                                            AED_List.get(i).getManager(),
                                            AED_List.get(i).getManagerTel(),
                                            AED_List.get(i).getMfg(),
                                            AED_List.get(i).getModel(),
                                            AED_List.get(i).getOrg(),
                                            AED_List.get(i).getLatitude(),
                                            AED_List.get(i).getLongitude(),
                                            AED_List.get(i).getZipcode1(),
                                            AED_List.get(i).getZipcode2(),
                                            AED_List.get(i).getDistance()));

                            //마커설정
                            options = new MarkerOptions();
                            options.position(
                                    new LatLng(
                                            Double.parseDouble(AED_List.get(i).getLatitude()),
                                            Double.parseDouble(AED_List.get(i).getLongitude())))
                                    .title(AED_List.get(i).getBuildAddress())
                                    .icon(BitmapDescriptorFactory.fromResource(R.drawable.pin2))
                                    .snippet(AED_List.get(i).getBuildAddress());

                            //지도에 마커 추가
                            Marker tempMarker = mGoogleMap.addMarker(options);
                            markerArrayList.add(tempMarker);
                        }
                        // Log.e("SOPT", AED_List.get(0).getBuildAddress());
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.aed_map, menu);
        return true;
    }
/*
    @Override
...
*/
    ...
    private class CheckTypeTasks extends AsyncTask<Void, Void, Void> {
        ...
        ProgressDialog progressDialog = new ProgressDialog(AedMapActivity.this);
